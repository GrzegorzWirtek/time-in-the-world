...
interface propsType {
    ...
    setState: React.Dispatch<React.SetStateAction<useLocaleStorageType | undefined>>
...
const Fieldset:React.FC<propsType> = ({state, setState, setMain}) => {
    ...
    const choosenCities = checked.map(item => 
        <li key={item} className="fieldset__chosen-li">{CITIES[item - 1].city}</li>
    )
    ...
    const handleSubmit = () => {
        const checkedCities = CITIES.filter(item => checked.indexOf(item.id) !== -1)
    ...
    const cities = CITIES.map(item => {
        ...
        return(
            ...
            <input 
             ...
             type="checkbox" id={item.city} 
             onChange={(e) => handleChange(e, item.id)} 
             checked={checked.includes(item.id)? true : false}/>
            <label className='fieldset__label' htmlFor={item.city}>{item.city}</label>
            <p className="fieldset__time">{cityHour}:{minutes}</p>
            </div>
            )
        })
    ...
    return(
        <fieldset className="fieldset">
            ...
            {cities}
            <button className="fieldset__button" onClick={handleSubmit}>Zatwierdź</button>
            <ul className="fieldset__chosen">
                {choosenCities}
            </ul>
        </fieldset>
    )
